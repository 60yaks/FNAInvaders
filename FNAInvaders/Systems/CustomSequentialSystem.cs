...
public sealed class CustomSequentialSystem<T> : ISystem<T>, IEnumerable<ISystem<T>>
{
    ...
    public CustomSequentialSystem(IEnumerable<ISystem<T>> systems)
    {
    ...
    /// <param name="systems">The <see cref="ISystem{T}"/> instances.</param>
    /// <exception cref="ArgumentNullException"><paramref name="systems"/> is null.</exception>
    public CustomSequentialSystem(params ISystem<T>[] systems)
